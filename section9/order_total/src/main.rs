    ...
    match (req.method(), req.uri().path()) {
        ...
        (&Method::POST, "/compute") => {
            ...
            if resp.status().as_u16() == 404 {
                let resp_body = ErrorResponse { status:"error".to_string(), message: "The zip code in the order does not have a corresponding sales tax rate.".to_string()};
                return Ok(response_build(&serde_json::to_string_pretty(&resp_body)?));
            }
